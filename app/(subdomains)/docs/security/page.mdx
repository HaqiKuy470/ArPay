...
**Property:** Every settlement either completes fully (community receives fiat) or fails safely (user receives full refund). No intermediate state exists where funds are lost.
...
∀ transaction T confirmed on-chain at slot s:
  ...
  Let F = event "community receives IDR"
  Let R = event "sponsor receives USDC refund"
...
- Only two exit paths from PDA: (a) release to treasury, (b) refund to sponsor

### 3. Cryptographic Commitment

**Property:** Once a transaction is confirmed on Solana, the commitment cannot be revoked without either satisfying the receiving community or refunding the sponsor.
...
| **Fiat Disbursement API (Xendit)** | Honest fiat disbursement | Community doesn't receive IDR |
...
- Program restricts `release_escrow` to:
  ...
  - Anyone after timeout (for refunds only back to original sponsor)
...
**Impact:** High (could release funds before community settlement confirmed)
...
- No arbitrage opportunity (USDC → IDR is one-way, community-specific)
...
Attacker replays TX
Solana tries to create PDA with seeds [payer, receiver, 12345]
→ PDA already exists
→ Transaction FAILS
```
...
### 4. Fiat Disbursement API Risks

### 4.1 Disbursement Failure

**Attack/Failure:** Xendit fails to disburse IDR to community after USDC is locked.
...
2. **Timeout refund:** If all retries fail, escrow times out and sponsor receives refund
...
→ After 120s total timeout, sponsor receives automatic refund
```

**Result:** ⚠️ **User inconvenience but no fund loss**.

### 4.2 Community Bank Account Invalid

**Attack:** Malicious community hub provides invalid bank details to trap funds.

**Impact:** Low (funds refunded to user)

**Mitigation:**
- **Community verification:** NMID mapped to verified bank account in database
- **Xendit validation:** Gateway validates bank account before disbursement
- **Failed disbursement → refund:** If bank details invalid, oracle refunds sponsor

**Result:** ✅ **Community cannot trap funds with invalid details**.
...
**Impact:** Low (transaction fails, sponsor retries)
...
- **User notification:** PWA displays "Network congested, please wait or retry" (for sponsor)
...
### 6.1 Community Regulatory Exposure

**Risk:** communities classified as Virtual Asset Service Providers (VASPs) under OJK/BI regulation.

**Mitigation:**
- **Isolation by design:** communities receive only IDR via licensed Fiat Disbursement API
- **No on-chain presence:** communities have no Solana wallet or crypto interaction
- **Fiat Disbursement API as shield:** Xendit is the regulated entity handling crypto conversion
- **Legal opinion:** Obtained from Indonesian fintech law firm confirming community hub exemption

**Result:** ✅ **communities are NOT considered VASPs** under current regulation.
...
**Mitigation:**
- **Community KYC:** All communities are QRIS-registered (BI-verified identities)
- **Transaction limits:** Enforce daily/monthly limits per community hub
...
4. **Regulatory compliance** (community isolation, AML/KYC)
